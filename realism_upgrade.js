...
function boot(){
  ...
  window.__thirdPerson=true;
  let localAvatar=null, thirdPerson=true, currentEmote='';
  const avatarParts={};
  ...
  function setThirdPerson(on){thirdPerson=!!on;window.__thirdPerson=thirdPerson;if(localAvatar)localAvatar.visible=thirdPerson;if(!thirdPerson){camera.position.set(controls.getObject().position.x,controls.getObject().position.y,controls.getObject().position.z);camera.rotation.set(0,0,0);}if(typeof showToast==='function')showToast(thirdPerson?'THIRD PERSON':'FIRST PERSON',1000);}
  localAvatar=makeAvatar();setThirdPerson(true);window.__localAvatar=localAvatar;

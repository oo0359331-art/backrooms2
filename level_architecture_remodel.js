...
function light(x,y,z,c=0xd2d5c6,int=.45,dist=18){const l=new T().PointLight(c,int,dist);l.position.set(x,y,z);group()?.add(l);return l;}
...
function level1(){
 const dark=0x3a3d3a, metal=0x555956;
 for(let x=-42;x<=42;x+=14)for(let z=-42;z<=42;z+=14){beam(x,4.25,z,.5,8.5,.5,dark);if((x+z)%28===0)panel(x,z,0xd7ded8);}
 ...
 box(14,.25,5.2,0x3e413e,0,3.65,18,0,.9,.15);beam(0,4.05,15,14,.25,.5,0x202321);stairs(-4,-18,10,4,.62,.28,0,0x4f514d);rail(-2,3.55,-14,4.5,0);rail(-2,3,-14,4.5,0);
 door(0,-44,0,0x315f65,1.85,2.7);windowStrip(-10,-43,5,1.3);windowStrip(10,-43,5,1.3);box(16,3.4,.35,0x454844,0,1.7,-5);box(.3,2.8,8,0x454844,7.8,1.4,-1);box(.3,2.8,8,0x454844,-7.8,1.4,-1);
...
function pool(deep=false){
 ...
 box(24,.15,10,water,0,.08,0,0,.18,.08);stairs(-32,-22,7,3.8,.62,.25,Math.PI/2,edge);rail(-29.8,2,-22,4,Math.PI/2);
 if(deep)for(let i=0;i<5;i++)pipe(-18+i*9,1,28,7,'z',0x42616b,.11);
...
function hotel(){
 ...
 for(let z=-40;z<=40;z+=20){beam(-20,4.45,z,.5,.4,.5,wood);beam(20,4.45,z,.5,.4,.5,wood);box(36,.18,6.6,0x5c4b3b,0,3.72,z,0,.92);}
...
function offices(){
 const glass=0x56767a, trim=0x303733;
 for(let x=-42;x<=42;x+=14){beam(x,4.4,0,.38,.65,88,trim);pipe(x,4,0,78,'z',0x4b514f,.12);}
 for(let z=-32;z<=32;z+=16)for(let x=-30;x<=30;x+=20){box(6.5,.09,4.3,0x6f7470,x,.05,z,0,.94);box(.08,2.5,4,glass,x-3.15,1.25,z);windowStrip(x,z-2.02,5.8,1.1,0);}
...
function tunnels(){
 ...
 for(let z=-48;z<=48;z+=16){box(2.8,1.5,.3,0x252824,0,1,z);box(2.1,.65,.12,0x6f7c72,0,1.55,z-.18);pipe(-7.2,2.8,z,.9,'y',0xa66c43,.08);pipe(7.2,2.8,z,.9,'y',0x4c7180,.08);}
 for(let z=-42;z<=42;z+=14)panel(0,z,0xb8c9b7);
}
function archive(){
 const wood=0x4a382e, trim=0x665846, paper=0xb6a98f;
 for(let x=-36;x<=36;x+=9)for(let z=-36;z<=36;z+=12){box(4,4.2,9.6,wood,x,2.1,z,0,.95);for(let y=1;y<=3;y++)for(let i=-1;i<=1;i++)box(.55,.05,7.5,paper,x+i*1.1,y,z-.1,0,.98);}
 box(10,.35,30,trim,0,4.35,0);rail(0,4.8,0,30,0,trim);stairs(-14,-30,9,4,.7,.27,Math.PI/2,wood);rail(-10.5,2.7,-27,5,Math.PI/2);panel(0,0,0xeeddb3);light(0,5.7,0,0xf2dfb6,.45,24);
}
function garden(){
 ...
 for(let x=-30;x<=30;x+=15)for(let z=-30;z<=30;z+=15)if((x+z)%30===0){cyl(1,2,green,x,1,z,18,.96);cyl(.72,1,0x5d8060,x,2.5,z,18,.97);}
 box(12,.18,3,stone,0,.1,0);arch(0,-32,5,3.2,0x9ebaae,0);door(0,-32,0,0x456a58,1.5,2.4);light(0,6,0,0xb9f0c8,.7,28);
}

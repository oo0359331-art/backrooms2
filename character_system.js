...
(function(){
  ...
  const state={seen:new WeakSet(),last:0};
  ...
  function pickAppearance(id){
    const p=profile();
    const seed=hash(String(id||p.name));
    const opts={body:['slim','average','broad'],hair:['short','side','long','hood'],clothes:['jacket','hoodie','utility']};
    const out={...p};
    if(!p.body||!opts.body.includes(p.body))out.body=opts.body[seed%opts.body.length];
    if(!p.hair||!opts.hair.includes(p.hair))out.hair=opts.hair[(seed>>>3)%opts.hair.length];
    if(!p.clothes||!opts.clothes.includes(p.clothes))out.clothes=opts.clothes[(seed>>>6)%opts.clothes.length];
    if(!/^#[0-9a-f]{6}$/i.test(out.pants))out.pants=DEFAULT.pants;
    if(!/^#[0-9a-f]{6}$/i.test(out.shoes))out.shoes=DEFAULT.shoes;
    return out;
  }
  function clearOld(group){
    const keep=[];
    group.children.slice().forEach(c=>{if(c.type==='Sprite'||c.name==='profileNameplate'||c.name==='localName')keep.push(c);else group.remove(c);});
    return keep;
  }
  function build(group,id,isLocal){
    if(!window.THREE||!group||group.userData.__brCharacterReady)return;
    clearOld(group);
    const p=pickAppearance(id);
    const root=new THREE.Group();root.name='br-character-model';
    const rig={root,arms:[],legs:[],baseY:0,phase:Math.random()*Math.PI*2,appearance:p};
    group.add(root);
    const scale=p.body==='slim'?.92:(p.body==='broad'?1.08:1);
    root.scale.set(scale,1,scale);
    const torsoH=p.body==='broad'?1.2:(p.body==='slim'?1.16:1.2);
    ...
    const head=new THREE.Mesh(new THREE.SphereGeometry(.32,16,12),mat(p.skin,.9));head.position.y=2.68;head.scale.set(.9,1.06,.9);head.castShadow=true;root.add(head);
    ...
    [-1,1].forEach(side=>{
      const arm=joint(side*.42,2.0,0);root.add(arm);
      arm.rotation.z=side*.035;
      ...
      const hand=new THREE.Mesh(new THREE.SphereGeometry(.105,10,8),mat(p.skin));hand.position.y=-.57;elbow.add(hand);
      rig.arms.push(arm);
    });
    ...
    group.userData.__brCharacterReady=true;
    group.userData.__brRig=rig;
    group.userData.__brProfileKey=JSON.stringify(p);
    group.userData.__brIsLocal=!!isLocal;
    animate(group,0);
  }
  function isMoving(group){
    ...
    const t=group.userData.targetPosition;
  ...
  function animate(group,t){
    ...
    if(rig.legs[0])rig.legs[0].rotation.x=a*amp;
    if(rig.legs[1])rig.legs[1].rotation.x=-a*amp;
    if(rig.arms[0])rig.arms[0].rotation.x=-a*amp*.9;
    if(rig.arms[1])rig.arms[1].rotation.x=a*amp*.9;
    rig.root.position.y=Math.sin(t*(moving?spd*2:1.7)+rig.phase)*(moving?.012:.004);
  }
  function apply(){
    const p=profile();
    safe(()=>{const av=window.__localAvatar;if(av){const k=JSON.stringify(p);if(av.userData.__brProfileKey!==k){av.userData.__brCharacterReady=false;build(av,p.name,true);}}});
    safe(()=>{if(typeof remotePlayers!=='undefined'&&remotePlayers)for(const [id,g] of remotePlayers){const k=JSON.stringify(p)+'|'+id;if(!g.userData.__brCharacterReady||g.userData.__brProfileOwner!==id){build(g,id,false);g.userData.__brProfileOwner=id;}}});
  }
  function ui(){
    ...
    const fill=()=>{const p=profile();['body','hair','clothes','pants','shoes'].forEach(k=>{const x=q('br-'+k);if(x)x.value=p[k]||DEFAULT[k];});};
    fill();
    ['body','hair','clothes','pants','shoes'].forEach(k=>q('br-'+k)?.addEventListener('change',()=>{const p=profile();p[k]=q('br-'+k).value;localStorage.setItem(KEY,JSON.stringify(p));safe(()=>q('profile-name')?.dispatchEvent(new Event('input')));build(window.__localAvatar,p.name,true);}));
  }

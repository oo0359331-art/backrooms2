...
(function(){
  ...
  function addStyle(){
    ...
    s.textContent=`
      ...
      #playability-mission .title{color:#ffd84d;letter-spacing:2px;margin-bottom:7px}
      #playability-mission .line{line-height:1.45;font-weight:600}
  ...
  function redLights(){
    ...
    const lights=[];
    scene.traverse(o=>{
      if(o.isLight){ lights.push({o,color:o.color.clone(),intensity:o.intensity}); o.color.set(0xff1408); o.intensity*=1.7; }
    });
    if(typeof scene.fog!=='undefined' && scene.fog && scene.fog.color){
      scene.__brFog=scene.fog.color.clone(); scene.fog.color.set(0x4b0505);
    }
    showHorror('THE LIGHTS ARE WRONG');
    setTimeout(()=>{
      lights.forEach(x=>{x.o.color.copy(x.color);x.o.intensity=x.intensity});
      if(scene.__brFog && scene.fog && scene.fog.color) scene.fog.color.copy(scene.__brFog);
    },1600);
  }
  ...
  function scheduleEvents(){
    ...
    const delay=26000+Math.random()*26000;
    lastEvent=setTimeout(()=>{randomHorrorEvent();scheduleEvents();},delay);
  }
  ...
  function makeBacteria(){
    ...
    const core=new THREE.Mesh(new THREE.IcosahedronGeometry(1.25,2),new THREE.MeshStandardMaterial({color:0x162416,emissive:0x0b260f,emissiveIntensity:1.15,roughness:.5}));
    bacteria.add(core);
    for(let i=0;i<10;i++){
      const p=new THREE.Mesh(new THREE.SphereGeometry(.22+Math.random()*.18,10,10),new THREE.MeshStandardMaterial({color:0x6be36d,emissive:0x153e18,emissiveIntensity:1.2,roughness:.4}));
      const a=(i/10)*Math.PI*2, r=1.05+Math.random()*.35; p.position.set(Math.cos(a)*r,(Math.random()-.5)*.8,Math.sin(a)*r); bacteria.add(p);
    }
    bacteria.add(new THREE.PointLight(0x70ff7a,2.2,9));
    const root=typeof controls!=='undefined'&&controls?controls.getObject().position:new THREE.Vector3();
    bacteria.position.set(root.x+28,1.3,root.z+24); scene.add(bacteria); bacteriaActive=true;
  }

  function updateBacteria(dt){
    if(!bacteria || !bacteriaActive || typeof controls==='undefined' || !controls) return;
    const p=controls.getObject().position; const d=bacteria.position.distanceTo(p);
    bacteria.rotation.y+=dt*1.4; bacteria.rotation.z+=dt*.8;
    const pulse=1+Math.sin(performance.now()*.005)*.08; bacteria.scale.setScalar(pulse);
    if(d<22){
      q('bacteria-label').style.display='block';
      const dir=p.clone().sub(bacteria.position); dir.y=0; if(dir.lengthSq()>0.01){dir.normalize(); bacteria.position.addScaledVector(dir,dt*2.6);}
    } else q('bacteria-label').style.display='none';
    if(d<3.5){
      showHorror('BACTERIA GOT TOO CLOSE');
      bacteria.position.set(p.x+18,1.3,p.z-22);
      if(typeof stamina!=='undefined') stamina=Math.max(15,stamina-35);
    }
  }
  ...
  function setupWideWorld(){
    ...
    window.__wideWorldPatched=true;
    try{updateInfiniteWorld(true);}catch(_){ }
  }

  function ensureExit(){
    if(typeof currentLevel==='undefined'||currentLevel==='2'||typeof exitObject!=='undefined' && exitObject) return;
    if(typeof createExitDoor==='function'){
      try{createExitDoor();}catch(_){ }
    }
  }

  function missionText(){
    const line=q('mission-line'); if(!line) return;
    if(typeof currentLevel==='undefined') return;
    if(typeof inventoryComplete==='function' && inventoryComplete()){
      line.innerHTML='<span class="done">✓ All 3 items found.</span><br>Go to the exit and press E.';
    } else {
      const count=(typeof inventory!=='undefined'&&Array.isArray(inventory))?inventory.length:0;
      line.textContent=`Find the 3 items. Progress: ${Math.min(3,count)}/3`;
    }
  }

  function enableReward(){
    setUnlocked();
    const b=q('hollow-purple-button');
    if(b){b.disabled=false;b.title='Unlocked by finishing a level.';b.style.display='block';}
    showHorror('HOLLOW PURPLE UNLOCKED');
    if(typeof showToast==='function') showToast('You finished the level. Hollow Purple is yours.');
  }

  function gateHollowPurple(){
    const b=q('hollow-purple-button');
    const ok=unlocked();
    if(b){b.disabled=!ok;if(ok)b.title='Unlocked by finishing a level.';}
    if(!ok && typeof document!=='undefined' && !window.__hollowGateBound){
      document.addEventListener('keydown',e=>{
        if(e.code!=='KeyP') return;
        e.preventDefault(); e.stopImmediatePropagation();
        if(typeof showToast==='function') showToast('Finish your current level to unlock Hollow Purple.');
      },true);
      window.__hollowGateBound=true;
    }
  }
  ...
  function wrapGame(){
    ...
    window.startGame=function(mode){
      ...
      setTimeout(()=>{
        setupMissionUI();
        const root=q('playability-mission'); if(root) root.style.display='block';
        try{ensureExit();}catch(_){ }
        try{setupWideWorld();}catch(_){ }
        try{if(typeof window.__bacteriaReset==='function')window.__bacteriaReset(); else {bacteriaActive=false;if(bacteria)bacteria.visible=false;makeBacteria();}}catch(_){ }
        gateHollowPurple(); scheduleEvents();
      },250);
    };
    ...
    if(typeof window.winGame==='function'){
      ...
      window.winGame=function(){
        ...
        const text=q('win-text'); if(text&&typeof currentLevel!=='undefined') text.textContent=`${username||'Player'} escaped Level ${currentLevel}.`;
        enableReward();
      };
    }
  ...
  function bindControls(){
    if(window.__controlWatchdog) return;
    window.__controlWatchdog=true;
    const down=e=>{
      if(typeof isMobileMode!=='undefined'&&isMobileMode) return;
      if(e.code==='KeyW') moveForward=true;
      else if(e.code==='KeyA') moveLeft=true;
      else if(e.code==='KeyS') moveBackward=true;
      else if(e.code==='KeyD') moveRight=true;
      else if(e.code==='ShiftLeft'||e.code==='ShiftRight'){if(typeof stamina==='undefined'||stamina>0)isSprinting=true;}
    };
    const up=e=>{
      if(e.code==='KeyW') moveForward=false;
      else if(e.code==='KeyA') moveLeft=false;
      else if(e.code==='KeyS') moveBackward=false;
      else if(e.code==='KeyD') moveRight=false;
      else if(e.code==='ShiftLeft'||e.code==='ShiftRight')isSprinting=false;
    };
  ...
  function frame(){
    if(typeof gameActive!=='undefined'&&gameActive && typeof controls!=='undefined'&&controls){
      const p=controls.getObject().position;
      if(lastPlayerPos){distanceTravelled+=p.distanceTo(lastPlayerPos);}
      lastPlayerPos=p.clone(); missionText();
      updateBacteria(1/60);
  ...
  function boot(){
    setupMissionUI(); bindControls();
    const tryWrap=()=>{if(!wrapGame())setTimeout(tryWrap,150);}; tryWrap();
    frame();
  }
